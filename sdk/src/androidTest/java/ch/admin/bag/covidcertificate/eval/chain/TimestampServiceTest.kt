...
class TimestampServiceTest {
	...
	fun future_expiration() {
		val dccHolder = DccHolder(emptyEuDgc, "", expirationTime = Instant.now().plusSeconds(60))
	...
	@Test
	fun no_expiration() {
		val dccHolder = DccHolder(emptyEuDgc, "", expirationTime = null)
		assertNull(TimestampService.decode(dccHolder))
	}
	...
	@Test
	fun past_issuedAt() {
		val dccHolder = DccHolder(emptyEuDgc, "", issuedAt = Instant.now().minusSeconds(60))
		assertNull(TimestampService.decode(dccHolder))
	}

	@Test
	fun no_issuedAt() {
		val dcc = DccHolder(emptyEuDgc, "", issuedAt = null)
